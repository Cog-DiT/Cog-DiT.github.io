...
good_examples = ["/work/koichi/Gear/gear_gen_angle_rad_con_auto/eval_results/Motion_10_MinRad20/Gear_Motion_Normalize_HighNoise_GearFixed160GeneralGraph_Depth160_From150_LR3e6_35400_SKIPLOW_MotionHighFixed160eralGraphDepth160From150LR3e6_35400_shift1_1.0_steps1_step32b/results/0_loop_0_modality_0.gif",
                 ...
                 "/work/koichi/Gear/gear_gen_angle_rad_con_auto/eval_results/Motion_Depth10_Node100_MinRad25_Efficient2/Gear_Motion_Normalize_HighNoise_GearFixed160GeneralGraph_Depth1100_From160_49800_SKIPLOW_MotionHighFixed160eralGraphDepth1100From160_49800_shift1_1.0_steps5_step32b/results/0_loop_0_modality_0.gif",
]
#################################################################
...
def _resolve_example(result_path, eval_data_root, reference_cache_dir):
    ...
    ground_truth_fallback = videos_root / "all_normalized_coordinate.mp4"
    driving_path = _first_existing([vis_dir / f"{vis_prefix}.mp4", driving_fallback])
    ground_truth_path = _first_existing([ground_truth_fallback])

    missing = []
    if driving_path is None:
        missing.append(f"driving condition: {driving_fallback}")
    if ground_truth_path is None:
        missing.append(f"ground truth (all gears): {ground_truth_fallback}")
    if missing:
        raise FileNotFoundError(
    ...
    return [
        ...
        ("Ground-Truth (output)", str(ground_truth_path)),
    ]
...
def visualize(grid_inputs, output_path):
    ...
    try:
        ...
        border_cfg = {
            ...
            "Ground-Truth (Target Output)": (255, 0, 0)    # Red border
        }
...
if __name__ == "__main__":
    output_dir = REPO_ROOT / "video" / "Task Definition"
